...
export function petFindPetsByStatus(
  ...
  Result<
    operations.FindPetsByStatusResponse | undefined,
    | PetstoreError
...
> {
  const input: operations.FindPetsByStatusRequest = {
    status: status,
  };
  ...
  const parsed = safeParse(
    input,
    (value) =>
      z.parse(operations.FindPetsByStatusRequest$outboundSchema, value),
  ...
  >(
    M.json(
      200,
      types$.optional(operations.FindPetsByStatusResponse$inboundSchema),
    ),
    M.bytes(
      200,
      types$.optional(operations.FindPetsByStatusResponse$inboundSchema),
