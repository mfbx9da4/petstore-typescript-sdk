...
import { PetstoreDefaultError } from "../models/errors/petstoredefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | PetstoreDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | PetstoreDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new PetstoreDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new PetstoreDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
